...
class SphinxEngine extends \Laravel\Scout\Engines\Engine
{
    ...
    public function update($models)
    {
        ...
        $models->each(function ($model) {
            if (!empty($searchableData = $model->toSearchableArray())) {
                if (isset($model->isRT)) { // 仅 RT 索引支持替换
    ...
    {
        return $this->performSearch($builder)->limit($perPage * ($page - 1), $perPage)->execute();
    }
    ...
    {
        if (isset($model->isRT)) { // 仅 RT 索引支持
            $index = $model->searchableAs();
            (new Helper($this->sphinx->getConnection()))->truncateRtIndex($index)->execute();
        }
    }
